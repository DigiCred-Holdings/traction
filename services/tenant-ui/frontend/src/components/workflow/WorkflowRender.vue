<template>
    <div class="card">
      ...
      <div v-for="block in currentState?.display_data" :key="JSON.stringify(block)" class="block">
                  <img v-if="block.type === 'image'" :src="block.url" class="image" />
                  <h3 v-else-if="block.type === 'text' && block.display === 'title'" class="title">{{ block.text }}</h3>
                  <p v-else-if="block.type === 'text'" class="text">{{ block.text }}</p>
                  <p v-else-if="block.type === 'extended'" class="extended">{{ block.text }}</p>
                  <Button
  ...
  <script setup lang="ts">
    ...
    console.log('WorkflowRender: Raw props received:', JSON.stringify(workflow.value, null, 2))
    ...
    console.log('WorkflowRender: displayData ref value:', JSON.stringify(workflow.value, null, 2))
    const defaultImage =
    ...
    watchEffect(() => {
        console.log('WorkflowRender: watchEffect triggered. Current displayData:', JSON.stringify(workflow.value, null, 2))
    })

    onMounted(() => {
      console.log('WorkflowRender: Component mounted. displayData:', JSON.stringify(workflow.value, null, 2));
    });
  </script>
